...
export default function CheckOutModule({ pets, user }) {
	...
	return (
		<div className='my-12 w-fit flex flex-col h-auto rounded-md ring-2  ring-gray-400 shadow-lg shadow-gray-400 '>
			<div className='flex items-center justify-center my-8 '>
				<h1 className='font-bold text-xl text-black'>Checking Out</h1>
			</div>
			{/* Pets being checked in for that day  will go here*/}
			<div className='flex flex-col  items-center w-full gap-1 mb-12 px-12'>
				{pets.map((pet) => (
					<div className='align-middle justify-center flex items-center p-2 px-12 my-2 text-gray-900 rounded-lg dark:text-white -700 ring-1 ring-gray-400 '>
						...
						>
							...
							/> */}
							<div className='w-full flex flex-col items-center justify-center py-1 mx-auto'>
							...
							<div className='flex gap-1'>
								<button onClick={handleArrived}>
									<TiTick className='text-4xl text-green-500' />
								</button>
								<button onClick={handleEdit}>
									<TiEdit className='text-4xl text-yellow-500' />
								</button>
								<button onClick={handleDelete}>
									<TiTrash className='text-4xl text-red-500' />
								</button>
							</div>
						</div>
					</div>
